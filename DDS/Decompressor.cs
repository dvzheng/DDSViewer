				...
				for (int z = 0; z < depth; z++) {
					for (int y = 0; y < height; y += 4) {
						for (int x = 0; x < width; x += 4) {
							...
							temp += 2;
							if (t1 > t2) {
								for (int i = 2; i < 8; ++i) yColours[i] = (byte)(t1 + ((t2 - t1) * (i - 1)) / 7);
							}  else {
								for (int i = 2; i < 6; ++i) yColours[i] = (byte)(t1 + ((t2 - t1) * (i - 1)) / 5);
								yColours[6] = 0;
								yColours[7] = 255;
							}
							...
							temp2 += 2;
							if (t1 > t2) {
								for (int i = 2; i < 8; ++i) xColours[i] = (byte)(t1 + ((t2 - t1) * (i - 1)) / 7);
							} else {
								for (int i = 2; i < 6; ++i) xColours[i] = (byte)(t1 + ((t2 - t1) * (i - 1)) / 5);
								xColours[6] = 0;
								xColours[7] = 255;
							}
									...
									if ((y + k + j) < height) {
										for (int i = 0; i < 4; i++) {
											...
											if (((x + i) < width)) {
												...
												if (t > 0) rawData[t1 + 2] = (byte)(Math.Sqrt(t) + 128);
												else rawData[t1 + 2] = 0x7F;
											}
											bitmask >>= 3;
											bitmask2 >>= 3;
										}
				...
				for (int z = 0; z < depth; z++) {
					for (int y = 0; y < height; y += 4) {
						for (int x = 0; x < width; x += 4) {
							...
							temp += 2;
							if (t1 > t2) {
								for (int i = 2; i < 8; ++i) colours[i] = (byte)(t1 + ((t2 - t1) * (i - 1)) / 7);
							} else {
								for (int i = 2; i < 6; ++i) colours[i] = (byte)(t1 + ((t2 - t1) * (i - 1)) / 5);
								colours[6] = 0;
								colours[7] = 255;
							}
