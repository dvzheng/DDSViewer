...
namespace Imaging.DDSReader.Utils {
	public class Helper {
		...
		internal static void ComputeMaskParams(uint mask, ref int shift1, ref int mul, ref int shift2) {
			...
			if (mask == 0 || mask == uint.MaxValue) return;
			while ((mask & 1) == 0) {
				mask >>= 1;
				shift1++;
			}
			...
			while ((mask * mul) < 255) mul = (mul << (int)bc) + 1;
			mask *= (uint)mul;
		...
		internal static uint CountBitsFromMask(uint mask) {
			...
			for (i = 0; i < 32; i++, testBit <<= 1) {
				if ((mask & testBit) != 0) {
					if (!foundBit) foundBit = true;
					count++;
				}
				else if (foundBit) return count;
			}
